...
import { KepalaRecord } from '../types';

export default function DataKepala() {
  const navigate = useNavigate();
  const [data, setData] = useState<KepalaRecord[]>([]);
  ...
  const handleExportCSV = () => {
    ...
    const headers = ['No', 'Jenjang', 'Jenis Prestasi', 'Nama Madrasah', 'NSM', 'Nama Kepala', 'L/P', 'Prestasi', 'Detail Pencapaian', 'Tingkat', 'Tanggal', 'Penyelenggara', 'Sertifikat'];
    ...
    link.href = url;
    link.setAttribute('download', `Data_Prestasi_Kepala_Madrasah_${new Date().toISOString().split('T')[0]}.csv`);
  ...
  return (
    <div className="min-h-screen bg-slate-50 pb-12">
      <Header title="Detail Prestasi Kepala Madrasah" />
      ...
      <main className="max-w-7xl mx-auto px-4 mt-8">
        ...
        <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-200 mb-6 flex flex-wrap gap-4 items-center">
          ...
          <select 
            ...
            className="px-3 py-1.5 border border-slate-300 rounded-lg text-sm outline-none focus:ring-2 focus:ring-amber-500"
          >
              ...
              <table className="w-full text-sm text-left animate-in fade-in duration-500">
                <thead className="bg-slate-50 text-slate-600 border-b border-slate-200">
                  <tr>
                    ...
                    <th className="px-4 py-3 min-w-[200px]">Nama Kepala</th>
                  ...
                  {filteredData.length > 0 ? (
                    filteredData.map((item, idx) => (
                      <tr key={item.id} className="hover:bg-slate-50">
                        ...
                        <td className="px-4 py-3">
                          <div className="font-medium text-slate-800">{item.namaKepala}</div>
                        ...
                        <td className="px-4 py-3 text-slate-600">
                          {item.sertifikatLink ? (
                            <a href={item.sertifikatLink} target="_blank" rel="noopener noreferrer" className="text-amber-600 hover:underline">Lihat Sertifikat</a>
                          ) : '-'}
                        </td>
                      </tr>
                    ))
                    ...
                    <tr>
                      <td colSpan={8} className="px-4 py-8 text-center text-slate-500">
                        Belum ada data prestasi kepala
                      </td>
                    </tr>
                  )}
                </tbody>
