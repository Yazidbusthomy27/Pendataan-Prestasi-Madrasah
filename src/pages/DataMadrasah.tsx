...
import { MadrasahRecord } from '../types';
...
export default function DataMadrasah() {
  ...
  const handleExportCSV = () => {
    ...
    const headers = ['No', 'Jenjang', 'Jenis', 'Nama Madrasah', 'NSM', 'Prestasi', 'Detail Pencapaian', 'Tingkat', 'Tanggal', 'Penyelenggara', 'Sertifikat'];
    ...
    link.href = url;
    link.setAttribute('download', `Data_Prestasi_Lembaga_Madrasah_${new Date().toISOString().split('T')[0]}.csv`);
  ...
  return (
    <div className="min-h-screen bg-slate-50 pb-12">
      <Header title="Detail Prestasi Lembaga Madrasah" />
                  ...
                  {filteredData.length > 0 ? (
                    filteredData.map((item, idx) => (
                      <tr key={item.id} className="hover:bg-slate-50">
                        ...
                        <td className="px-4 py-3 text-purple-600 font-medium">{item.prestasi}</td>
                        ...
                        <td className="px-4 py-3 text-slate-600">
                          {item.sertifikatLink ? (
                            <a href={item.sertifikatLink} target="_blank" rel="noopener noreferrer" className="text-purple-600 hover:underline">Lihat Sertifikat</a>
                          ) : '-'}
                        </td>
                      </tr>
                    ))
                    ...
                    <tr>
                      <td colSpan={9} className="px-4 py-8 text-center text-slate-500">
                        Belum ada data prestasi madrasah
                      </td>
                    </tr>
                  )}
                </tbody>
